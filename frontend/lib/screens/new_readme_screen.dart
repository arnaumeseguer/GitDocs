...
class _NewReadmeScreenState extends State<NewReadmeScreen> {
  ...
  Future<void> _generate() async {
    ...
    try {
      ...
      const fallbackOrder = ['gemini', 'grok', 'claude'];
      const modelNames = {'gemini': 'Gemini', 'grok': 'Grok', 'claude': 'Claude'};
      final order = [_aiModel, ...fallbackOrder.where((m) => m != _aiModel)];
      ...
      for (final model in order) {
        setState(() => _loadMsg = 'Generant amb ${modelNames[model] ?? model}…');
        try {
          ...
          if (model == order.last) throw lastError;
        }
      }
